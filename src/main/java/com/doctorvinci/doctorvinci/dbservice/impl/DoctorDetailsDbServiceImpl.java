...
public class DoctorDetailsDbServiceImpl implements DoctorDetailsDbService {
 ...
 private final AddressDbService addressDbService;
  public DoctorDetailsDbServiceImpl(DoctorDetailsRepository doctorDetailsRepository, AddressDbService addressDbService) {
  ...
  public void saveDoctorDetailInDb(DoctorDetails doctorDetails) {
    ...
    @Override
    public void saveHospitalDoctorDetails(DoctorDetails doctorDetails, int hospitalId, int hospitalAddressId) {
        DoctorDetailsEntity doctorDetailsEntity = DoctorDetailsEMM.map(doctorDetails);
    ...
    @Override
    public void saveHospitalDoctorDetails(List<DoctorDetails> doctorDetailsList, int hospitalId, int hospitalAddressId) {
  ...
  public List<DoctorDetails> retrieveDoctorDetailsByDoctorTypeAndCity(
     DoctorType doctorType, String city) {
       ...
       */
      List<DoctorDetailsEntity> doctorDetailsEntities = doctorDetailsRepository.findByDoctorTypeAndAddressIdIn
       ...
       */
      for(DoctorDetailsEntity doctorDetailsEntity: doctorDetailsEntities){// doctorDetailEntity1 having addressId 2
      ...
      doctorDetailsList.add(doctorDetails); // doctorDetailsList me hum doctorDetail1 add kar rahe hain
      } // ab yahi cheez doctorDetailEntity2 k liye repeat hogi
      return doctorDetailsList ;
  }

    @Override
    public List<DoctorDetails> getDoctorsInLatitudeAndLongitudeRange(double latitudeStart, double latitudeEnd,
    ...
    @Override
    public List<DoctorDetails> getDoctorsInLatitudeAndLongitudeRangeByDoctorType(double latitudeStart, double latitudeEnd, double longitudeStart, double longitudeEnd, DoctorType doctorType) {
        return null;
    ...
    public List<DoctorDetails> getDoctorsByHospitalId(int hospitalId, Address hospitalAddress) {
        List<DoctorDetails> doctorDetailsList = new ArrayList();
       List<DoctorDetailsEntity> doctorDetailsEntities= doctorDetailsRepository.findByHospitalId(hospitalId);
